...
class SvgAxis {
  ...
  _create(Element e, SelectionScope scope) {
    ...
    var tickFormat = this.tickFormat == null
            ? current.tickFormat(suggestedTickCount)
            : this.tickFormat,
        tickValues = this.tickValues == null
            ? current.ticks(suggestedTickCount)
            : this.tickValues;
      ...
      case ORIENTATION_BOTTOM:
        {
          ...
          textEnter
            ..attr('dy', '.71em')
            ..style('text-anchor', 'middle');
          ellipsis..attr('x', axisLength / 2)..attr('y', '.71em');
          path.attr('d',
      ...
      case ORIENTATION_TOP:
        {
          ...
          textEnter
            ..attr('dy', '0em')
            ..style('text-anchor', 'middle');
          ellipsis..attr('x', axisLength / 2)..attr('y', 0);
          path.attr('d',
              'M${range[0]},${-outerTickSize}V0H${range[1]}V${-outerTickSize}');
        }
        break;
    ...
    if (orientation == ORIENTATION_BOTTOM || orientation == ORIENTATION_TOP) {
      num maxTextWidth = 0, textHeight = text.first.clientHeight;
      text.each(
          (d, i, e) => maxTextWidth = math.max(maxTextWidth, e.clientWidth));
      ...
      text.each((d, i, e) {
        preTransX
            .add(prevRotate > 0 ? e.clientWidth * math.cos(prevArc) / 2 : 0);
        nowTransX
            .add(currentRotate > 0 ? e.clientWidth * math.cos(nowArc) / 2 : 0);
    ...
    tickTransform(
        tickUpdate, transformFn != null ? transformFn : current.apply);
  }
