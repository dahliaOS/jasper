...
EasingFn clampEasingFn(EasingFn f) => (t) => t <= 0 ? 0 : t >= 1 ? 1 : f(t);

// Ease-in is defined is the identifyFunction.
/** Ease-out */
EasingFn reverseEasingFn(EasingFn f) => (t) => 1 - f(1 - t);
...
EasingFn easeBack([s = 1.70158]) => (num t) => t * t * ((s + 1) * t - s);

EasingFn easeQuad() => (num t) => t * t;

EasingFn easeCubic() => (num t) => t * t * t;

EasingFn easeCubicInOut() => (num t) {
      if (t <= 0) return 0;
      if (t >= 1) return 1;
      var t2 = t * t, t3 = t2 * t;
...
EasingFn easeSin() => (num t) => 1 - math.cos(t * math.pi / 2);

EasingFn easeExp() => (num t) => math.pow(2, 10 * (t - 1));

EasingFn easeCircle() => (num t) => 1 - math.sqrt(1 - t * t);

EasingFn easeBounce() => (num t) => t < 1 / 2.75
    ? 7.5625 * t * t
    : t < 2 / 2.75
